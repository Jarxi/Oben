  ...
  try {
    ...
    if (req.body.type === 'time' || req.body.type === 'expense') {
      ...
      if (submission.type === 'time') {
        ...
        return res.status(500).json({
          message:
            'Can only submit time or expense. Please check submission type'
        });
      }
  ...
  try {
    try {
      ...
      if (submission.status != 'pending') {
        ...
        const update = await Submission.findOneAndUpdate(
          { _id: req.body._id },
          { $set: { status: 'returned', note: req.body.note } }
        );
        if (!update) {
  ...
  try {
    ...
    if (user && startDate && endDate) {
      const selectedSubmissions = await Submission.find({
        submitter: user,
        input: { dateAmount: { date: { $gte: startDate, $lte: endDate } } }
      });
      return res.status(200).json({
...
const update = async (req, res) => {
  try {
    ...
    for (let i = 0; i < req.body.input.length; i++) {
      ...
      const update = await Submission.findOneAndUpdate(
        { submitter: user, type: type },
        { $set: { 'input.$[outer].dateAmount.$[inner].amount': amount } },
        {
          arrayFilters: [
            { 'outer.project_name': project_name },
            { 'inner.date': date }
          ]
        }
      );
